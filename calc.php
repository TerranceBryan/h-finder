...
if(isset($_REQUEST['calc'])) {
    ...
    if(isset($_REQUEST['amount'], $_REQUEST['month'], $_REQUEST['interest'])) {
        ...
        if($mon <= 0) {
            echo "Number of months must be greater than zero.";
            exit; // Stop execution
        }

        // Calculate interest and payment per month
        $interest = $amount * ($int / 100);
        $pay = $amount + $interest;
        
        // Check if the number of months is zero, to avoid division by zero
        if($mon != 0) {
            $month = $pay / $mon;
            echo "Monthly payment: " . number_format($month, 2);
        } else {
            echo "Number of months cannot be zero.";
        }
    } else {
